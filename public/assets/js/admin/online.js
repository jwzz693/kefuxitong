...
// 将指定频道的对话移到列表顶部（即时视觉反馈）
function moveToTop(channel) {
    var $item = $("#v" + channel);
    if ($item.length === 0) return;
    $("#chat_list").prepend($item);
    $item.find('.c_time').text('刚刚');
}

// 防抖的 getchat，避免短时间内多次刷新
var _getchatTimer = null;
function debouncedGetchat(delay) {
    delay = delay || 800;
    if (_getchatTimer) clearTimeout(_getchatTimer);
    _getchatTimer = setTimeout(function() {
        _getchatTimer = null;
        getchat();
    }, delay);
}
            ...
            if(res.code ==0){
                if(res.data){
                    ...
                    if(res.data.state == 'online'){
                        $("#v_state").html("<span class='v-state-badge online'>在线</span>");
                    }else{
                        $("#v_state").html("<span class='v-state-badge offline'>离线</span>");
                    }
                }
            }
        }
            ...
            if (res.code == 0) {
                ...
                $.each(data, function (k, v) {
                    ...
                    if (debug == v.visiter_id) {
                        ...
                        var isOnline = (v.state == 'online');
                        var grayClass = isOnline ? '' : ' icon_gray';
                        ...
                        a += "<div class='visit_content' onclick='choose(" +v.vid+ ")'><img class='am-radius v-avatar" + grayClass + "' id='img" +v.channel + "' src='" + v.avatar + "' width='50px'><span class='status-dot " + dotClass + "' id='dot" + v.channel + "'></span>"+name+"<span class='c_time'>" + v.timestamp + "</span><div id='msg" +v.channel  + "' class='newmsg'>"+v.content+"</div>";
                        a += '</div></div>';
                        ...
                        var noticeHtml;
                        if(v.count == 0){
                            noticeHtml = '<span id="c' +v.channel + '" class="notice-icon hide"></span>';
                        }else{
                            var cnt = v.count > 99 ? "99+" : v.count;
                            noticeHtml = '<span id="c' +v.channel + '" class="notice-icon">'+cnt+'</span>';
                        }
                        a += '<div id="v' + v.channel + '" class="visiter" onmouseover="showcut(this)" onmouseout="hidecut(this)"><i class="layui-icon myicon hide" title="删除" style="font_weight:blod" onclick="cut(' + "'" + v.visiter_id + "'" + ')">&#x1006;</i>' + noticeHtml;
                        a += "<div class='visit_content' onclick='choose(" +v.vid+ ")'><img class='am-radius v-avatar" + grayClass + "' id='img" + v.channel + "' src='" + v.avatar + "'  width='50px'><span class='status-dot " + dotClass + "' id='dot" + v.channel + "'></span>"+name+"<span class='c_time'>" + v.timestamp + "</span><div id='msg" + v.channel + "' class='newmsg'>"+v.content+"</div>";
                        a += '</div></div>';
        ...
        success: function (res) {
            ...
            if (res.code == 0) {
                ...
                $.each(res.data, function (k, v) {
                    ...
                    var dotClass = isOnline ? 'online' : 'offline';
                    a += '<div class="waiter">';
                    a += '<img id="img'+v.visiter_id+'" class="am-radius w-avatar v-avatar' + grayClass + '" src="' + v.avatar + '" width="50px" height="50px"><span class="status-dot ' + dotClass + '"></span><span class="wait_name">' + uname + '</span>';
        ...
        success: function (res) {
            if (res.code == 0) {
                ...
                $.each(data, function (k, v) {
                    ...
                    a += ' <span style="font-size: 14px;color: #555555;line-height: 80px;margin-left: 82px">' + v.visiter_name + '</span><div style="position:absolute;right:0;top:30px;cursor: pointer;" onclick="recovery(' + "'" + v.visiter_id + "'" + ')"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#F43F5E" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></div></div>';
                });
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if (res.code == 0) {
                ...
                str += '</div><div class="group-list-left"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--primary, #4F46E5)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="editusergroup" data-vid="'+user.vid+'"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="16"></line><line x1="8" y1="12" x2="16" y2="12"></line></svg></div><div class="group-list-right">';
                ...
                btnClass=user.istop?'layui-btn-danger':'layui-btn-normal';
                 ...
                 str +='<a class="layui-btn layui-btn-sm  chatexport chat2top js-ajax-btn"  href="/admin/index/export?visiter_id='+user.visiter_id+'" target=_blank "><i class="layui-icon layui-icon-screen-full"></i>导出记录</a>';
                  str +='<a class="layui-btn layui-btn-sm   chat2top js-ajax-btn"  onclick="clearList()" target=_blank "><i class="layui-icon  layui-icon-delete"></i>清空会话列表</a>';
                 str +='</div></div></div></div>';
                $.each(data, function (k, v) {
                    if (v.cid < mindata) {
                        mindata = v.cid;
