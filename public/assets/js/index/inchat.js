        ...
        $("#picture").ajaxSubmit({
            ...
            success: function (res) {
                if(res.code == 0){
                    ...
                    $("img[src*='upload/images']").not('.payment-card-qr img').parent().parent('.customer').css({
                        padding: '0',borderRadius: '0',maxHeight:'100px'
                    });
                    $("img[src*='upload/images']").not('.payment-card-qr img').parent().parent('.service').css({
                        padding: '0',borderRadius: '0',maxHeight:'100px'
                    });
                    setTimeout(function(){
                        $('.chatmsg').css({
...
function getdata(){
    ...
    $.ajax({
            ...
            if(res.code == 0){
                ...
                $.each(res.data, function (k, v) {
                    ...
                    if (v.direction == 'to_service') {
                        ...
                        if (v.content.indexOf('payment-card-wrap') > -1) {
                            str += v.content;
                        } else {
                            str += "<pre>" + v.content + "</pre>";
                        }
     ...
     //gettishi($.cookie('services'));
          setTimeout('hint()',600)
}

function gettishi(business_id){

    $.ajax({
        url:ROOT_URL+'/admin/event/gettishi',
        type:'post',
        ...
        success:function(res){
            if(res.code == 0){
              ...
              var d = new Date(),
                ...
                console.log(num)
      if(num == 0){
               var strtime  = '<div class="showtime"  style="margin-bottom: 30px;">'+$('#services').text() + v[2]+'&nbsp;&nbsp;'+time+'</div>'
                      $(".conversation").append(strtime);
        
                        num =1
          
      }
                var str='';
                str += '<li class="chatmsg"><div style="position: absolute;left: 3px;">';
                str += '<img  class="my-circle" src="/assets/images/index/ai_service.png" width="40px" height="40px"></div>';
                str += "<div class='outer-left' '><div class='service'>";
                 str += "<pre><i class='layui-icon "+v[0]+"' style='font-size: 22px;cursor: pointer;'></i>&nbsp;" ;    
                num =num+1;
     ...
     setTimeout('var div = document.getElementById("wrap");div.scrollTop = div.scrollHeight;',600)
        setTimeout('document.getElementById("chat-message-audio").play();',1500)
       
       
}

function zhuanrengong(business_id){
    $.ajax({
        url:ROOT_URL+'/admin/event/zhuanrengong',
        type:'post',
        data:{business_id:business_id,visiter_id:visiter_id},
        success:function(res){
            if(res.code == 0){
                ...
                str += '<li class="chatmsg"><div style="position: absolute;left: 3px;">';
                str += '<img  class="my-circle" src="/assets/images/index/ai_service.png" width="40px" height="40px"></div>';
                str += "<div class='outer-left'><div class='service'><p style='font-weight:blod;'>你好！已成功为您连线人工，请选择您的客服专员:</p></br>";
                str += "<pre><ul>" ;
                var num ='';
                ...
                $.each(res.data,function(k,v){
                    ...
                    str += '<a href="/index/index/home?visiter_id=&visiter_name=&avatar=&business_id='+business_id+'&groupid='+v[2]+'&special='+v[0] +'" style="color: rgb(0, 176, 240); text-decoration:none;"><span style="color: rgb(0, 176, 240);">【'+v[1]+'】</span></a>'
              //      str+='<li style="list-style:none;float: left;  color:#13c9cb; " onclick="tz('+business_id +','+v[3] +',' +v[0]+')">【'+v[1]+'】</li>';
...
function tz(business_id,groupid,id){
    console.log(business_id)
        console.log(id)
        window.location.href = ROOT_URL + '/index/index/home?visiter_id=&visiter_name=&avatar=&business_id='+business_id+'&groupid='+groupid+'&special='+id
}
//获取tab
...
$(document).on('click','.outer-left .service img,.outer-right .customer img',function(e) {
    if ($(this).closest('.payment-card-wrap').length > 0) return;
    var that = this;
    var img = new Image();
    img.src = this.src
    // 如果图片被缓存，则直接返回缓存数据
