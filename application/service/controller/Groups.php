...
class Groups extends Base
{
    ...
    public function edit()
    {
        if ($this->request->isAjax()) {
            ...
            $res = Group::where("id", $post['id'])->where('business_id',$_SESSION['Msg']['business_id'])->field(true)->update($post);
            if ($res) $this->success('修改成功');
            $this->error('修改失败！');
        }
        $id = $this->request->get('id');
        $group = Group::where(['id' => $id,'business_id'=>$_SESSION['Msg']['business_id']])->find();
    ...
    public function remove()
    {
        ...
        if (Group::destroy(['id' => $id,'business_id'=>$_SESSION['Msg']['business_id']])) $this->success('操作成功！');
        $this->error('操作失败！');
    }
}