...
class Setting extends Base
{
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $update = ['lang' => $post['lang'],'pctab' => $post['pctab'], 'auto_trans' => $post['auto_trans'], 'auto_ip' => $post['auto_ip'], 'template_state' => $post['template_state'], 'ts1' => $post['ts1'], 'ts2' => $post['ts2'], 'ts3' => $post['ts3'], 'audio_state' => $post['audio_state'], 'location_state' => $post['location_state'], 'chat_tpl' => isset($post['chat_tpl']) ? $post['chat_tpl'] : 'default'];
