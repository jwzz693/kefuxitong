...
use app\admin\controller\Fanyi;
...
$str = str_replace("xss","",$str);
$arr['content']=($str);
}else{
$arr['content']=self::safe_filter($arr['content']);
        ...
        if ($service_id != $arr['service_id']) {
            ...
            if (!empty($service_id)) {
                ...
                $returndata = ['code' => 0,'cid' => $cid, 'msg' => 'success'];
                return $returndata;
            } else {
                // 客服关闭了对话框，重新设置为打开
                $data = ['state' => 'normal'];
        ...
        try {
            ...
            $chatModel = new Chats();
            $chid=$chatModel->allowField(true)->save($arr);
            if($chid){
            ...
            $data = ['code' => 0,'cid' => $chid, 'msg' => 'success', 'data' => ['getui' => isset($getui_ret) ? $getui_ret : (isset($getui_e) ? $getui_e : '')]];
...
//        $ip = $_SERVER["REMOTE_ADDR"];
        ...
        public function gettishi()
    {
        ...
        if ($result) {
            $arr = ['code' => 0, 'msg' => 'success', 'data' => $ts];
            return $arr;
        }
    }
    public function zhuanrengong()
    {
        $post = $this->request->post();
        $business_id = $post['business_id'];
    ...
    public function checkchat()
    {
        ...
        $data =Admins::table('wolive_chats')->where(['service_id'=>$service_id,'visiter_id'=>$post['visiter_id'],'business_id'=>$business_id])->order('timestamp desc')->select();
        foreach ($data as $v) {
            if($v['direction'] == 'to_service'){
                if(!$v['is_read']){
                    $code=1;
