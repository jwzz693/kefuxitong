...
//        $arr = $this->request->post('',null,null);
        ...
        $queue = Db::table('wolive_queue')
            ...
            ->where('business_id',$login['business_id'])
            ->find();
        if(isset($arr['type']) && $arr['type']=='input'){
            $pusher->trigger("cu" . $channel, 'input-event', array('message' => $arr));
...
//wolive_chats 消息表
    ...
    public function getwatch()
    {
        ...
        $data = Admins::table('wolive_chats')->where(['service_id' => $service_id, 'visiter_id' => $post['visiter_id'],'direction' => 'to_service','is_read' => 0, 'business_id' => $login['business_id']])->select();
        foreach ($data as $value){
            Admins::table('wolive_chats')->where('cid', $value['cid'])->update(['is_read' => 1]);
            $cids[]=$value['cid'];
        }
        if(!empty($cids)){
            if (!ahost) {
...
//D:\web\kfsystem\public\assets\css\index\ai_service.css
    ...
    public function createMinjs()
    {
        ...
        try {
             ...
             */
                      ...
                      var moblieweb = encodeURI('{$url}{$this->base_root}/mobile/index/home?{$theme}visiter_id='+blzx.visiter_id+'&visiter_name='+blzx.visiter_name+'&avatar='+blzx.avatar+'&business_id={$business_id}&groupid='+groupid+'&product='+blzx.product);
                       ...
                       if(!s){
                            ...
                            {$xfbg}
                                                if(blzx.isMobile()){
                               div.style.width='100%';
                  div.innerHTML='<i class='blzx-close2\" onclick=\"tanchuang2()\"></i><iframe id=\"wolive-iframe\" src=\"'+web+'\" onload=\"pageOk()\"></iframe><div id=\"loading\"><img src=\"{$url}{$this->base_root}/assets/images/platform/loading-2.svg\"></div>'
                                document.body.appendChild(div);     
                           }
                                ...
                                if(!blzx.isMobile()){
                                    ...
                                    div.innerHTML='<i class=\"blzx-close\" onclick=\"tanchuang()\"></i><i class=\"blzx-close2\" onclick=\"tanchuang2()\"></i><iframe id=\"wolive-iframe\" src=\"+web+\"onload=\"pageOk()\"></iframe><div id=\"loading\"><img src=\"{$url}{$this->base_root}/assets/images/platform/loading-2.svg\"></div>'
                                          document.body.appendChild(div);
                            }
                ...
                function tanchuang(){
                    window.open(\"{$url}{$this->base_root}/index/index/home?visiter_id=&visiter_name=&avatar=&business_id={$business_id}&groupid=0\",\"_blank\",\"height=800,width=950,top=50,left=200,status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=no,location=no,titlebar=no\")
    ...
    public function createMinDiyjs()
    {
        ...
        $is_type = $this->request->post('isopen_win');
        if($type == 2){
            $div = "testt";
            $mdiv = "";
            $hdiv = "";
    ...
    if($is_type == 'on'){
            $div_type = "minblzxmsgtitlecontainerlabel";
        }else{
            $div_type = "minblzxmsgtitlecontainerlabel";
        }
        $login = $_SESSION['Msg'];
     ...
     $file = file_put_contents(ROOT_PATH . "/public/assets/layer/ai_service_diy_" . $login['business_id'] . ".js", "
             ...
             */
                ...
                visiter_id:(typeof ai_service=='undefined' || typeof ai_service.visiter_id == 'undefined')?'':ai_service.visiter_id,
                     ...
                     open:function(){
                        ...
                        if(!d){
                            ...
                            w.classList.add('{$div}');
                            w.innerHTML='<div id=\"minblzxmsgtitlecontainer_yuan\"  onclick=\"blzx.connenct(0)\"><img id=\"minblzxWinlogo_yuan\" src=\"{$url}{$this->base_root}/assets/style1/img/wechatLogo.png\"><i class=\"layui-icon\" id=\"minblzxNewBigWin\"  onclick=\"blzx.connenct(0)\"></i></div>';
                            document.getElementById('minblzxmsgtitlecontainer_yuan').style.backgroundColor='{$xfbg}';
                     ...
                     document.getElementById('blzxMinChatWindowDiv_yuan').style.display=\"none\";
                      var id =groupid;
                      var web =encodeURI('{$url}{$this->base_root}/layer?{$theme}visiter_id='+blzx.visiter_id+'&visiter_name='+blzx.visiter_name+'&avatar='+blzx.avatar+'&business_id={$business_id}&groupid='+groupid+'&product='+blzx.product+ '&special=' + {$service_id});
                      
                      var moblieweb = encodeURI('{$url}{$this->base_root}/mobile/index/home?{$theme}visiter_id='+blzx.visiter_id+'&visiter_name='+blzx.visiter_name+'&avatar='+blzx.avatar+'&business_id={$business_id}&groupid='+groupid+'&product='+blzx.product + '&special=' + {$service_id});
                       var s =document.getElementById('wolive-talk');
                       ...
                       if(!s){
                            ...
                            if(blzx.isMobile()){
                               ...
                               {$mheight}
                                                               ...
                                                               if(blzx.isMobile()){
                               div.style.width='100%';
                  div.innerHTML='<i class=\"blzx-close2 layui-icon layui-icon-close\" onclick=\"tanchuang2()\"></i><iframe id=\"wolive-iframe\" src=\"'+web+'\" onload=\"pageOk()\"></iframe><div id=\"loading\"><img src=\"{$url}{$this->base_root}/assets/images/platform/loading-2.svg\"></div>'
                                     document.body.appendChild(div);     
                           }
                                ...
                                if(!blzx.isMobile()){
                                    ...
                                    document.getElementById('blzxMinChatWindowDiv_yuan').style.right='6px';
                                    document.getElementById('blzxMinChatWindowDiv_yuan').style.margin='15px';
                                    div.innerHTML='<i class=\"blzx-close\" onclick=\"tanchuang()\"></i><i class=\"blzx-close2\" onclick=\"tanchuang2()\"></i><i class=\"blzx-close2 layui-icon layui-icon-close\" onclick=\"tanchuang2()\"></i><iframe id=\"wolive-iframe\" src='+web+' onload=\"pageOk()\"></iframe><div id=\"loading\"><img src=\"{$url}{$this->base_root}/assets/images/platform/loading-2.svg\"></div>'
                                               document.body.appendChild(div);
                            }
                ...
                function tanchuang(){
                    window.open(\"{$url}{$this->base_root}/index/index/home?visiter_id=&visiter_name=&avatar=&business_id={$business_id}&groupid=0+ &special={$service_id}\",\"_blank\",\"height=800,width=950,top=50,left=200,status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=no,location=no,titlebar=no\")
                    blzx.closeMinChatWindow('wolive-talk')
                ...
                visiter_id:(typeof ai_service=='undefined' || typeof ai_service.visiter_id == 'undefined')?'':ai_service.visiter_id,
                     ...
                     open:function(){
                        ...
                        if(!d){
                            ...
                            w.classList.add('{$div}');
                            w.innerHTML='<div id=\"minblzxmsgtitlecontainer\"><img id=\"minblzxWinlogo\" src=\"{$url}{$this->base_root}/assets/style1/img/wechatLogo.png\"><div id=\"{$div_type}\" class=\"{$mdiv}\" onclick=\"blzx.connenct(0)\">在线咨询</div><i class=\"layui-icon \" id=\"minblzxNewBigWin\"  onclick=\"blzx.connenct(0)\"></i></div>';
    ...
    public function pushPayment()
    {
        ...
        if (!empty($arr['payment_id'])) {
            $paymentIds = explode(',', $arr['payment_id']);
            $payments = Db::table('wolive_payment_method')
                ->where('business_id', $login['business_id'])
        ...
        if (empty($payments)) {
            return json([
                'code' => 1,
                'msg' => '暂无可用的支付方式，请先前往设置'
            ]);
        }

        $channel = bin2hex($arr['visiter_id'] . '/' . $login['business_id']);
        $pusher->trigger("cu" . $channel, 'push-payment', array('message' => $payments));

        // 同时在聊天中发送一条支付卡片消息
        $payHtml = '<div class="payment-card-wrap">';
        foreach ($payments as $p) {
            $payHtml .= '<div class="payment-card-item">';
            $payHtml .= '<div class="payment-card-name">' . htmlspecialchars($p['method_name']) . '</div>';
            if (!empty($p['account_info'])) {
                $payHtml .= '<div class="payment-card-account">账号: ' . htmlspecialchars($p['account_info']) . '</div>';
            }
            if (!empty($p['qrcode_url'])) {
                $payHtml .= '<div class="payment-card-qr"><img src="' . htmlspecialchars($p['qrcode_url']) . '" alt="收款码"></div>';
            }
            if (!empty($p['payment_link'])) {
                $linkUrl = htmlspecialchars($p['payment_link']);
                $payHtml .= '<div class="payment-card-link"><a href="' . $linkUrl . '" target="_blank" rel="noopener noreferrer" class="payment-card-link-btn">点击前往付款 →</a></div>';
            }
            $payHtml .= '</div>';
        }
        $payHtml .= '</div>';
        ...
        try {
            $cid = Db::table('wolive_chats')->insertGetId($chatData);
            $chatData['avatar'] = $login['avatar'];
            $chatData['cid'] = $cid;
            $chatData['tip'] = '';
            $pusher->trigger("cu" . $channel, 'my-event', array('message' => $chatData));
        } catch (\Exception $e) {
            // 发送聊天记录失败不影响推送
        }
...
if($arr['service_id']!=$_SESSION['Msg']['service_id']){
    ...
    public function pushMarqueeAd()
    {
        ...
        // 获取指定的广告或所有启用的广告
        if (!empty($arr['ad_id'])) {
            $adIds = explode(',', $arr['ad_id']);
            $ads = Db::table('wolive_marquee_ad')
                ->where('business_id', $login['business_id'])
                ->where('id', 'in', $adIds)
                ->where('status', 1)
                ->order('sort desc, id desc')
        ...
        if (empty($ads)) {
            return json([
                'code' => 1,
                'msg' => '暂无可用的流动广告，请先前往设置'
            ]);
        }

        $channel = bin2hex($arr['visiter_id'] . '/' . $login['business_id']);
        $pusher->trigger("cu" . $channel, 'push-marquee-ad', array('message' => $ads));

        // 同时在聊天中发送一条提示消息
        $adText = '';
        foreach ($ads as $ad) {
            $adText .= '📢 ' . htmlspecialchars($ad['content']);
            if (!empty($ad['link_url'])) {
                $adText .= ' <a href="' . htmlspecialchars($ad['link_url']) . '" target="_blank" style="color:#4F46E5;">[查看详情]</a>';
            }
            $adText .= '<br>';
        }
        $chatContent = '<div class="marquee-ad-msg" style="background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:#fff;padding:10px 15px;border-radius:10px;font-size:13px;">'
            . '<div style="font-weight:600;margin-bottom:4px;"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:-2px;margin-right:4px;"><path d="M22 17H2a1 1 0 0 0 0 2h20a1 1 0 0 0 0-2z"/><path d="M2 7h20a1 1 0 0 0 0-2H2a1 1 0 0 0 0 2z"/><path d="M6 12h12"/></svg>流动广告推送</div>'
            . $adText
            . '</div>';
