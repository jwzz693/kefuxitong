...
class Pusher extends Worker 
{
    ...
    /**
     * 自动升级检测间隔（秒），默认 3600 秒（1小时）
     * 设为 0 禁用自动升级检测
     *
     * @var int
     */
    public $autoUpgradeInterval = 3600;
    ...
    public function onStart()
    {
        ...
        // 自动升级检测定时器
        if ($this->autoUpgradeInterval > 0) {
            // 首次延迟 60 秒后检测，之后按设定间隔周期检测
            Timer::add(60, array($this, 'autoUpgradeCheck'), null, false);
            Timer::add($this->autoUpgradeInterval, array($this, 'autoUpgradeCheck'));
            echo "Auto-upgrade check enabled, interval: {$this->autoUpgradeInterval}s\n";
        }
    }
    ...
    /**
     * 自动升级检测
     * 检测云端是否有新版本，若有则执行自动升级并重启服务
     */
    public function autoUpgradeCheck()
    {
        $basePath = dirname(dirname(__DIR__));
        $scriptPath = $basePath . '/auto_upgrade.php';

        if (!file_exists($scriptPath)) {
            echo date('Y-m-d H:i:s') . " Auto-upgrade script not found: {$scriptPath}\n";
            return;
        }

        echo date('Y-m-d H:i:s') . " Running auto-upgrade check...\n";

        $isWindows = strtoupper(substr(PHP_OS, 0, 3)) === 'WIN';

        if ($isWindows) {
            // Windows: 异步执行升级脚本
            pclose(popen("start /B php \"{$scriptPath}\" upgrade", 'r'));
        } else {
            // Linux: 异步执行升级脚本
            exec("php {$scriptPath} upgrade > /dev/null 2>&1 &");
        }
    }
}
